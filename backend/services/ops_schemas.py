"""Schemas Pydantic do backend (Fase 1 corrigida).

Motivo dessa duplicacao com `sdk/winegod_scraper_sdk/schemas.py`:
- O backend roda em Docker/Render com WORKDIR=backend. O SDK NAO esta no
  PYTHONPATH nesse runtime.
- A auditoria Codex detectou que o fallback anterior aceitava payloads sem
  validacao, violando Design Freeze v2.
- Correcao: backend tem seus proprios schemas, aqui, com `extra='forbid'`,
  sem nenhum fallback permissivo.

Este arquivo DEVE ficar estritamente alinhado com o SDK. Quem alterar um
precisa alterar o outro na mesma release (ou via sub-projeto de build
unificado).
"""
...
class EndRunPayload(_StrictModel):
    ...
    # Contadores opcionais precisam ser >= 0 quando preenchidos.
    @field_validator(
...
# ---------------------------------------------------------------------------
# Registry de modelos por nome — usado pelo router.
# ---------------------------------------------------------------------------

MODELS = {
    "ScraperRegisterPayload": ScraperRegisterPayload,
    "StartRunPayload":        StartRunPayload,
    "HeartbeatPayload":       HeartbeatPayload,
    "EndRunPayload":          EndRunPayload,
    "FailRunPayload":         FailRunPayload,
    "EventPayload":           EventPayload,
    "BatchEventPayload":      BatchEventPayload,
