...
def test_parse_registry_tags_extracts_plug_and_pending():
    from routes.ops_dashboard import parse_registry_tags
    meta = parse_registry_tags(["review", "plug:reviews_scores", "pending:contract"])
    assert meta["plug"] == "reviews_scores"
    assert meta["pending"] == "contract"


def test_classify_run_mode_distinguishes_dry_run_and_blocked():
    from routes.ops_dashboard import classify_run_mode
    assert classify_run_mode({"dry_run": True}, "observed") == "dry_run"
    assert classify_run_mode({"dry_run": False}, "observed") == "apply"
    assert classify_run_mode({}, "blocked_external_host") == "blocked"
