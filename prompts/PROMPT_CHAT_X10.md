...
# CHAT X10 — Deduplicacao de Vinhos (Grupo 10 de 10)
...
Filtrar SEMPRE: `WHERE pais_tabela IN ('co', 'fi', 'hu', 'jp', 'lu', 'bg', 'ru', 'il', 'ge', 'cz', 'cn', 'ae', 'kr', 'no', 'hr', 'tw', 'tr', 'th')`
...
```sql
CREATE TABLE IF NOT EXISTS wines_unique_g10 (
...
CREATE TABLE IF NOT EXISTS dedup_quarantine_g10 (
    id SERIAL PRIMARY KEY,
...
Pares com match_probability entre 0.50 e 0.80 vao pra `dedup_quarantine_g10`:
...
- Progresso: `[X10] pais US: 784,300 vinhos | nivel 1: 520K agrupados | nivel 2: processando...`

## O QUE NAO FAZER

- **NAO processar paises que NAO sao seus** — so os listados acima
- **NAO modificar wines_clean** — so ler
- **NAO criar tabela wines_unique** (sem sufixo) — so wines_unique_g10
...
```bash
cd C:\winegod-app && python scripts/dedup_group_10.py
```

Verificar:
```sql
SELECT COUNT(*) FROM wines_unique_g10;
SELECT match_type, COUNT(*) FROM wines_unique_g10 GROUP BY match_type;
SELECT total_copias, COUNT(*) FROM wines_unique_g10 GROUP BY total_copias ORDER BY total_copias DESC LIMIT 10;
SELECT COUNT(*) FROM dedup_quarantine_g10;
```
...
```
=== GRUPO 10 CONCLUIDO ===
Paises: CO, FI, HU, JP, LU, BG, RU, IL, GE, CZ, CN, AE, KR, NO, HR, TW, TR, TH
...
Output: X vinhos unicos em wines_unique_g10
