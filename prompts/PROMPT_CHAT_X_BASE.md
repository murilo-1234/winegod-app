...
# CHAT X{GROUP_NUM} — Deduplicacao de Vinhos (Grupo {GROUP_NUM} de 10)
...
Filtrar SEMPRE: `WHERE pais_tabela IN ({COUNTRY_SQL})`
...
Criar um script `scripts/dedup_group_{GROUP_NUM}.py` que:
1. Le vinhos do seu grupo da tabela `wines_clean`
2. Deduplica em 3 niveis (deterministico + probabilistico + quarentena)
3. Salva resultado em `wines_unique_g{GROUP_NUM}` e `dedup_quarantine_g{GROUP_NUM}`
...
```sql
CREATE TABLE IF NOT EXISTS wines_unique_g{GROUP_NUM} (
...
CREATE TABLE IF NOT EXISTS dedup_quarantine_g{GROUP_NUM} (
    id SERIAL PRIMARY KEY,
...
Pares com match_probability entre 0.50 e 0.80 vao pra `dedup_quarantine_g{GROUP_NUM}`:
...
- Progresso: `[X{GROUP_NUM}] pais US: 784,300 vinhos | nivel 1: 520K agrupados | nivel 2: processando...`

## O QUE NAO FAZER

- **NAO processar paises que NAO sao seus** — so os listados acima
- **NAO modificar wines_clean** — so ler
- **NAO criar tabela wines_unique** (sem sufixo) — so wines_unique_g{GROUP_NUM}
...
```bash
cd C:\winegod-app && python scripts/dedup_group_{GROUP_NUM}.py
```

Verificar:
```sql
SELECT COUNT(*) FROM wines_unique_g{GROUP_NUM};
SELECT match_type, COUNT(*) FROM wines_unique_g{GROUP_NUM} GROUP BY match_type;
SELECT total_copias, COUNT(*) FROM wines_unique_g{GROUP_NUM} GROUP BY total_copias ORDER BY total_copias DESC LIMIT 10;
SELECT COUNT(*) FROM dedup_quarantine_g{GROUP_NUM};
```
...
Output: X vinhos unicos em wines_unique_g{GROUP_NUM}
