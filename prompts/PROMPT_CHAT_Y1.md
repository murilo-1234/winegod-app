...
# CHAT Y1 — Match Vivino (Grupo 1/8: us)

Voce e 1 de 8 abas rodando em paralelo. Seu grupo: **1** (us, ~545K vinhos).
...
```bash
cd C:\winegod-app && python scripts/match_vivino.py 1
```

O script:
1. Le vinhos de wines_unique WHERE pais_tabela IN ('us')
2. Busca match contra vivino_match (1.7M vinhos Vivino importados localmente)
3. Salva resultado em match_results_y1
