...
# CHAT Y13 — Match Vinhos de Loja contra Vivino (Grupo 13 de 15)
...
```
wines_unique WHERE id >= 2353837 AND id <= 2549989
```
...
Criar script `scripts/match_vivino_13.py` que:
...
4. Salva resultado em `match_results_g13` (banco local)
...
```sql
CREATE TABLE IF NOT EXISTS match_results_g13 (
...
CREATE INDEX idx_mr13_uid ON match_results_g13 (unique_id);
CREATE INDEX idx_mr13_vid ON match_results_g13 (vivino_id) WHERE vivino_id IS NOT NULL;
CREATE INDEX idx_mr13_level ON match_results_g13 (match_level);
```
...
df_loja = pd.read_sql("""
    ...
    FROM wines_unique
    WHERE id >= 2353837 AND id <= 2549989
""", conn_local)
...
- Progresso: `[Y13] Nivel 1: X matches | Nivel 2: processando... | Sem match: X`

## O QUE NAO FAZER

- **NAO processar IDs fora da sua faixa**
- **NAO modificar wines_unique nem wines (Render)** — so ler
- **NAO criar tabela match_results** (sem sufixo) — so match_results_g13
...
```bash
cd C:\winegod-app && python scripts/match_vivino_13.py
```

Verificar:
```sql
SELECT match_level, COUNT(*), ROUND(AVG(match_probability)::numeric, 2) as prob_media
FROM match_results_g13 GROUP BY match_level ORDER BY COUNT(*) DESC;

SELECT COUNT(*) as total FROM match_results_g13;
SELECT COUNT(*) FILTER (WHERE vivino_id IS NOT NULL) as com_match FROM match_results_g13;
SELECT COUNT(*) FILTER (WHERE vivino_id IS NULL) as sem_match FROM match_results_g13;
```
...
```
=== GRUPO Y13 CONCLUIDO ===
Input: X vinhos de wines_unique (IDs 2353837 a 2549989)
...
Tabela: match_results_g13 populada com X registros
```
