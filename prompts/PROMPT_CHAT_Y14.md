...
# CHAT Y14 — Match Vinhos de Loja contra Vivino (Grupo 14 de 15)
...
```
wines_unique WHERE id >= 2549990 AND id <= 2746142
```
...
Criar script `scripts/match_vivino_14.py` que:
...
4. Salva resultado em `match_results_g14` (banco local)
...
```sql
CREATE TABLE IF NOT EXISTS match_results_g14 (
...
CREATE INDEX idx_mr14_uid ON match_results_g14 (unique_id);
CREATE INDEX idx_mr14_vid ON match_results_g14 (vivino_id) WHERE vivino_id IS NOT NULL;
CREATE INDEX idx_mr14_level ON match_results_g14 (match_level);
```
...
df_loja = pd.read_sql("""
    ...
    FROM wines_unique
    WHERE id >= 2549990 AND id <= 2746142
""", conn_local)
...
- Progresso: `[Y14] Nivel 1: X matches | Nivel 2: processando... | Sem match: X`

## O QUE NAO FAZER

- **NAO processar IDs fora da sua faixa**
- **NAO modificar wines_unique nem wines (Render)** — so ler
- **NAO criar tabela match_results** (sem sufixo) — so match_results_g14
...
```bash
cd C:\winegod-app && python scripts/match_vivino_14.py
```

Verificar:
```sql
SELECT match_level, COUNT(*), ROUND(AVG(match_probability)::numeric, 2) as prob_media
FROM match_results_g14 GROUP BY match_level ORDER BY COUNT(*) DESC;

SELECT COUNT(*) as total FROM match_results_g14;
SELECT COUNT(*) FILTER (WHERE vivino_id IS NOT NULL) as com_match FROM match_results_g14;
SELECT COUNT(*) FILTER (WHERE vivino_id IS NULL) as sem_match FROM match_results_g14;
```
...
```
=== GRUPO Y14 CONCLUIDO ===
Input: X vinhos de wines_unique (IDs 2549990 a 2746142)
...
Tabela: match_results_g14 populada com X registros
```
