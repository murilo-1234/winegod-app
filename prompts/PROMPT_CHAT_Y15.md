...
```
wines_unique WHERE id >= 2746143 AND id <= 2942304
```
Estimativa: ~196,162 vinhos.
...
Criar script `scripts/match_vivino_15.py` que:
...
4. Salva resultado em `match_results_g15` (banco local)
...
```sql
CREATE TABLE IF NOT EXISTS match_results_g15 (
...
CREATE INDEX idx_mr15_uid ON match_results_g15 (unique_id);
CREATE INDEX idx_mr15_vid ON match_results_g15 (vivino_id) WHERE vivino_id IS NOT NULL;
CREATE INDEX idx_mr15_level ON match_results_g15 (match_level);
```
...
df_loja = pd.read_sql("""
    ...
    FROM wines_unique
    WHERE id >= 2746143 AND id <= 2942304
""", conn_local)
...
- Progresso: `[Y15] Nivel 1: X matches | Nivel 2: processando... | Sem match: X`

## O QUE NAO FAZER

- **NAO processar IDs fora da sua faixa**
- **NAO modificar wines_unique nem wines (Render)** — so ler
- **NAO criar tabela match_results** (sem sufixo) — so match_results_g15
...
```bash
cd C:\winegod-app && python scripts/match_vivino_15.py
```

Verificar:
```sql
SELECT match_level, COUNT(*), ROUND(AVG(match_probability)::numeric, 2) as prob_media
FROM match_results_g15 GROUP BY match_level ORDER BY COUNT(*) DESC;

SELECT COUNT(*) as total FROM match_results_g15;
SELECT COUNT(*) FILTER (WHERE vivino_id IS NOT NULL) as com_match FROM match_results_g15;
SELECT COUNT(*) FILTER (WHERE vivino_id IS NULL) as sem_match FROM match_results_g15;
```
...
```
=== GRUPO Y15 CONCLUIDO ===
Input: X vinhos de wines_unique (IDs 2746143 a 2942304)
...
Tabela: match_results_g15 populada com X registros
```
