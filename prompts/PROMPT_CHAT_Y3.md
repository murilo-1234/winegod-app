...
# CHAT Y3 — Match Vivino (Grupo 3/8: gb, hk, be, gr, hu, bg, hr)

Voce e 1 de 8 abas rodando em paralelo. Seu grupo: **3** (gb, hk, be, gr, hu, bg, hr, ~343K vinhos).
...
```bash
cd C:\winegod-app && python scripts/match_vivino.py 3
```

O script:
1. Le vinhos de wines_unique WHERE pais_tabela IN ('gb', 'hk', 'be', 'gr', 'hu', 'bg', 'hr')
2. Busca match contra vivino_match (1.7M vinhos Vivino importados localmente)
3. Salva resultado em match_results_y3
