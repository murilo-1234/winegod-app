...
# CHAT Y7 — Match Vivino (Grupo 7/8: de, dk, ph, ie, in, ge, tw)

Voce e 1 de 8 abas rodando em paralelo. Seu grupo: **7** (de, dk, ph, ie, in, ge, tw, ~343K vinhos).
...
```bash
cd C:\winegod-app && python scripts/match_vivino.py 7
```

O script:
1. Le vinhos de wines_unique WHERE pais_tabela IN ('de', 'dk', 'ph', 'ie', 'in', 'ge', 'tw')
2. Busca match contra vivino_match (1.7M vinhos Vivino importados localmente)
3. Salva resultado em match_results_y7
