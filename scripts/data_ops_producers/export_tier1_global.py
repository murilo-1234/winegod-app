"""CLI Tier1 global (semanal, APIs/sitemap deterministico).

Gera JSONL + summary em `reports/data_ops_artifacts/tier1/`. Read-only.
Nao aplica. Retorna exit=2 com `no_producer_tier1_global` se o
`lojas_scraping` nao tiver lojas Tier1 cadastradas.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Exporter Tier1 global (APIs/sitemap).")
    ...
    parser.add_argument("--country-filter", default=None, help="CSV ISO-2 (ex: us,fr)")
    parser.add_argument(
        "--methods",
        default=None,
