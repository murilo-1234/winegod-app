...
Faixa: wines_unique IDs 1961531 a 2157683
"""
...
def create_output_table(conn):
    ...
    """)
    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_mr11_uid ON {TABLE_OUT} (unique_id);")
    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_mr11_vid ON {TABLE_OUT} (vivino_id) WHERE vivino_id IS NOT NULL;")
    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_mr11_level ON {TABLE_OUT} (match_level);")
    conn.commit()
    cur.close()
    ...
    if os.path.exists(cache_path):
        ...
        for attempt in range(5):
            try:
                ...
                log(f"  Cache salvo em: {cache_path}")
                break
            except Exception as e:
                log(f"  Tentativa {attempt+1}/5 falhou: {e}")
                if attempt == 4:
                    raise
                import time as _t
