...
Faixa: wines_unique IDs 2353837 a 2549989
"""
...
def create_output_table(conn):
    ...
    """)
    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_mr13_uid ON {TABLE_OUT} (unique_id);")
    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_mr13_vid ON {TABLE_OUT} (vivino_id) WHERE vivino_id IS NOT NULL;")
    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_mr13_level ON {TABLE_OUT} (match_level);")
    conn.commit()
    cur.close()
...
def load_data():
    ...
    while True:
        sql = f"""
            ...
            FROM wines ORDER BY id LIMIT {chunk_size} OFFSET {offset}
        """
        for attempt in range(max_retries):
            try:
                conn_render = psycopg2.connect(
                    RENDER_URL,
                    connect_timeout=60,
                    keepalives=1,
                    keepalives_idle=30,
                ...
                log(f"  ERRO chunk offset={offset} tentativa {attempt+1}: {e}")
                try:
                    conn_render.close()
                except Exception:
                    pass
                if attempt < max_retries - 1:
                    wait = 10 * (attempt + 1)
                    log(f"  Aguardando {wait}s antes de retry...")
                    time.sleep(wait)
                else:
                    raise
        if not rows:
        ...
        offset += chunk_size
        log(f"  chunk {len(chunks)}: {len(rows):,} vinhos (total: {sum(len(c) for c in chunks):,})")
