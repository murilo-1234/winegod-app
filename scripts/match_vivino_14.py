...
Faixa: wines_unique IDs 2549990 a 2746142
"""
...
def create_output_table(conn):
    ...
    """)
    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_mr14_uid ON {TABLE_OUT} (unique_id);")
    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_mr14_vid ON {TABLE_OUT} (vivino_id) WHERE vivino_id IS NOT NULL;")
    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_mr14_level ON {TABLE_OUT} (match_level);")
    conn.commit()
    cur.close()
...
def load_data():
    ...
    log("Carregando wines Vivino (Render) via LIMIT/OFFSET...")
    ...
    def connect_render():
        for attempt in range(5):
            try:
                ...
                log(f"  Conexao tentativa {attempt+1}/5 falhou: {e}")
                if attempt < 4:
    ...
    while True:
        for attempt in range(3):
            try:
                ...
                log(f"  Chunk offset={offset} tentativa {attempt+1}/3 falhou: {e}")
                if attempt < 2:
                    time.sleep(10)
                else:
                    raise
        ...
        offset += chunk_size
        log(f"  ... {offset:,} vinhos lidos do Vivino")
