"""
CHAT Y4 — Match Vinhos de Loja contra Vivino (Grupo 4 de 15)
Faixa: wines_unique IDs 588460 a 784612
"""
...
def create_output_table(conn):
    ...
    """)
    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_mr4_uid ON {TABLE_OUT} (unique_id);")
    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_mr4_vid ON {TABLE_OUT} (vivino_id) WHERE vivino_id IS NOT NULL;")
    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_mr4_level ON {TABLE_OUT} (match_level);")
    conn.commit()
    cur.close()
...
def load_data():
    ...
    for attempt in range(3):
        try:
            conn_render = psycopg2.connect(RENDER_URL, connect_timeout=30, keepalives=1,
            ...
            log(f"  Tentativa {attempt+1}/3 falhou: {e}")
            if attempt == 2:
                raise
            import time as _t
            _t.sleep(5)
