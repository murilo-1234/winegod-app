"""
CHAT Y6 — Match Vinhos de Loja contra Vivino (Grupo 6 de 15)
Faixa: wines_unique IDs 980766 a 1176918
"""
...
def create_output_table(conn):
    ...
    """)
    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_mr6_uid ON {TABLE_OUT} (unique_id);")
    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_mr6_vid ON {TABLE_OUT} (vivino_id) WHERE vivino_id IS NOT NULL;")
    cur.execute(f"CREATE INDEX IF NOT EXISTS idx_mr6_level ON {TABLE_OUT} (match_level);")
    conn.commit()
    cur.close()
    ...
    while True:
        for attempt in range(8):
            try:
                ...
                log(f"  Tentativa {attempt+1}/8 falhou (offset={offset}): {e}")
                try:
                    conn_render.close()
                except:
                    pass
        ...
        time.sleep(1)  # pausa entre batches pra nao sobrecarregar
