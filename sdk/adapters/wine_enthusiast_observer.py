"""Observer read-only: Wine Enthusiast persistido em winegod_db."""
...
MANIFEST_PATH = HERE / "manifests" / "critics_wine_enthusiast.yaml"
...
def run(dry_run: bool = True, limit: int = 0) -> int:
        ...
        BatchEventPayload.model_validate(
            {
                ...
                "source_lineage": {
                    "source_system": "wine_enthusiast",
                    "source_kind": "table",
                    "source_pointer": "we_vinhos+we_queries+we_exec_*",
    ...
    if not dsn:
        reporter.event(
            ...
            message="WINEGOD_DATABASE_URL nao configurado para Wine Enthusiast",
            level="warn",
        )
        reporter.fail(
    ...
    try:
        with SafeReadOnlyClient(dsn) as client:
            if not table_exists(client, "we_vinhos"):
                reporter.event(
                    code="source.schema_missing",
                    message="we_vinhos ausente",
                    level="error",
                )
                reporter.fail(error_summary="source_schema_missing: we_vinhos", status="failed")
                return 4

            total_wines = count_rows(client, "we_vinhos")
            queries_total = count_rows(client, "we_queries") if table_exists(client, "we_queries") else 0
            score_rows = client.fetchone(
                "SELECT count(*) FROM public.we_vinhos WHERE we_score IS NOT NULL"
            )
            scored_reviews = int(score_rows[0] or 0) if score_rows else 0
            reviewers_total = count_distinct(client, "we_vinhos", "we_reviewer")
            ...
            exec_tables = list_tables(client, "we_exec_%")
            exec_total = 0
            recent_execs_7d = 0
            latest_exec = None
        ...
        reporter.batch_metrics(
            ...
            items_valid_local=scored_reviews,
            items_sent=scored_reviews,
            items_final_inserted=0,
            source_lineage={
                "source_system": "wine_enthusiast",
                "source_kind": "table",
                ...
                "notes": (
                    ...
                    f"recent_execs_7d={recent_execs_7d}; reviewers_total={reviewers_total}; "
                    f"latest_seen={latest_seen or latest_exec or 'unknown'}"
                )[:256],
            },
        )
        reporter.event(
            code="wine_enthusiast.snapshot",
            message=(
                f"wines={total_wines} scored_reviews={scored_reviews} "
                f"reviewers={reviewers_total} queries={queries_total} exec_total={exec_total} "
        ...
        print(
            f"[{manifest.scraper_id}] run OK wines={total_wines} scored_reviews={scored_reviews} "
