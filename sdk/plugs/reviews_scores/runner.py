    ...
    if reporter:
        ...
        reporter.batch_metrics(
            ...
            source_lineage={
                ...
                "notes": f"source={source}"[:256],
            },
        )
        reporter.event(
            code="plug.reviews_scores.summary",
            message=f"source={source} items={len(bundle.items)}",
            level="audit",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Plug de staging para reviews e scores")
    ...
    if args.apply:
        parser.error("reviews_scores e staging-only nesta sessao; use --dry-run ou o default")
